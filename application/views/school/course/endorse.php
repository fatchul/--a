...
<div class="modal-content">
    <div class="modal-header">
      ...
      <h4 class="modal-title" onclick="asu()"><b><?php echo $title ?></b></h4>
    </div>
    <div class="modal-body" >
      <div class="row">
        ...
        <div class="col-md-4 gets">
            <p align="center"><b>Guru Terdaftar Dalam Course Ini</b></p>
             <table class="table ">
                <thead>
                  <tr class="text-center">
                    
                    <th>Nama</th>                    
                    <th><span onclick="o()" class="btn btn-xs">Hapus</span></th>
                  </tr>
                </thead>
                  ...
                  <?php if ($listed_course<>0):  ?>
                    <?php foreach ($listed_course as $key => $v):  ?>
                      <tr id="rows<?php echo $v->id ?>">
                        ...
                        <td align="center">
                          <a class="btn btn-xs btn-danger " href='javascript:void()' title="Hapus" onclick="del(<?php echo $v->id ?>)"><i class="glyphicon glyphicon-trash"></i></a>
                        </td>
                      </tr>   
                    <?php endforeach ?>
                  <?php endif ?>
...
<script type="text/javascript">
  function save()
  {
      ...
      $.ajax({
          url : "<?php echo site_url('school/course/save')?>",
          type: "POST",
          data: data,
          dataType: "JSON",
  ...
  function del(id){
    if(confirm('Are you sure delete this data?')){
      $.ajax({
        url : "<?php echo site_url('school/course/revoke')?>/"+id,
        type: "GET",
        dataType: "JSON",
        success: function(data){
           $('#rows'+id).remove();
